...
export default function ReadyToCreate() {
  ...
  function toComposer(e: React.MouseEvent<HTMLAnchorElement>) {
    ...
    if (el) {
      ...
      // Nudge focus to the URL field so the visitor can type immediately.
      window.setTimeout(() => document.getElementById('hero-url')?.focus(), 500)
    }
  }
  ...
  return (
    <section className="relative overflow-hidden bg-[#0E1320] px-5 py-24 sm:py-28">
      ...
      <Reveal className="relative mx-auto max-w-2xl text-center">
        <span className="inline-block rounded-full border border-[#3B82F6]/40 bg-[#3B82F6]/10 px-3 py-1 text-xs font-medium text-[#9CC0F8]">
          Your launch, on autopilot
        </span>
        <h2 className="mt-5 text-4xl font-semibold tracking-tight text-white sm:text-5xl">
          Ready to create?
        </h2>
        <p className="mx-auto mt-4 max-w-md text-[#AEB7C6]">
          Paste your URL and let Filmo plan, price, and produce a finished launch video.
        </p>

        <a
          href="/#start"
          onClick={toComposer}
          className="group mt-8 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#2563EB] to-[#5AA0FB] px-7 py-3.5 text-base font-semibold text-white shadow-[0_18px_50px_-16px_rgba(59,130,246,0.85)] transition hover:from-[#1D4FD8] hover:to-[#4E94F7] hover:shadow-[0_22px_60px_-16px_rgba(59,130,246,1)]"
        >
          Start creating
          <svg
            viewBox="0 0 24 24"
