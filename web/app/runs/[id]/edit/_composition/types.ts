...
export interface SceneData {
  ...
  // real brand logos for split-mosaic/logo-wall tiles, index-aligned with featureEntities; data URIs staged at build time; '' = fallback to name
  entityLogos?: string[];
...
export interface TimelineData {
  ...
  theme: Theme;
  scenes: Scene[];
  // HOSTED EDITOR ONLY (not emitted by the pipeline): base URL the in-browser
  // preview prepends to public-relative asset names (screenshots / VO / music)
  // so they resolve to the InsForge `walk-videos` bucket / CDN instead of
  // Remotion's render-time public/. ABSENT = staticFile fallback (studio path).
  assetBaseUrl?: string;
  [key: string]: unknown;
}
