...
// THEME-TOKEN ONLY. Logo seam matches split-mosaic (web resolveLogo+resolveSrc).
// ---------------------------------------------------------------------------
...
const ScanGridTile: React.FC<{
  ...
  pillArrive: number;
  resolveSrc?: (p: string) => string;
}> = ({ entity, logo, pill, theme, arrive, scannedAmount, pillArrive, resolveSrc }) => (
  <div
    style={{
      position: "relative",
      borderRadius: 14,
  ...
  >
    {logo ? (
      <Img src={resolveLogo(logo, resolveSrc)} style={{ width: 46, height: 46, objectFit: "contain", borderRadius: 8 }} />
    ) : (
      <div
        style={{
          width: 44,
...
const TreatmentScanGrid: React.FC<{
  ...
  data, theme, frame, fps, cues, kickerAt, titleAt, subAt,
  titleOpacity, titleContainerY, underline, titleText, titleLines, sceneId, resolveSrc,
}) => {
  // Caller guarantees a non-empty grid (degrade guard runs before mount).
